#' Umbrella function to map HS codes to CPC codes
#'
#' The function takes trade data set with mapping table and finds corresponding
#' CPC codes for specific reporter, flow and HS code combinations.
#'
...
#'
#' @return Data frame with unique combinations reporter/flow/hs/cpc.
#'
#' @import dplyr
#' @export

mapHS2CPC <- function(tradedata,
                      maptable,
                      hs6maptable,
                      year = NULL,
                      parallel = FALSE) {
  ...
  hs2cpc_mapped_links <- uniqhs
  rprt_writetable(hs2cpc_mapped_links, prefix = tradedataname)

  # Report on nolinks
  rprt_hs2cpc_nolinks(uniqhs, tradedataname = tradedataname)

  # Report on multilinks
  rprt_hs2cpc_multilinks(uniqhs, tradedataname = tradedataname)

  flog.trace("HS+ mapping: selection from multiple matches", name = "dev")

  uniqhs <- sel1CPC(uniqhs, maptable, cur_yr = year)
