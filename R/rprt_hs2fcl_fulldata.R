...
rprt_hs2fcl_fulldata <- function(tradedata, tradedataname = NULL) {
  ...
  byreporter <- tradedata %>%
    group_by_(~reporter) %>%
    summarise_(.dots = stats)
  
  byreporterhs6 <- tradedata %>% 
  ...
  rprt_writetable(byreporter, prefix = tradedataname)
  rprt_writetable(byreporterhs6, prefix = tradedataname)
  rprt_writetable(byhs6, prefix = tradedataname)
