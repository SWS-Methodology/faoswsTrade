...
##' # Get Data from Complete TF CPC
##'
##' Get monetary values and quantities from data previously
...
flagWeightTable_status <- frame_data(
  ...
  # 'X',                   1.00, # This was never the case
  # '',                    0.99,
  'A',                    0.99,
  # 'T',                   0.80,
  'X',                   0.80,
...
flagWeightTable_status_no_imp <-
  ...
  mutate(flagObservationWeights = ifelse(flagObservationStatus == "I", 1.00, flagObservationWeights))
...
if (remove_nonexistent_transactions == TRUE) {
  ...
  new_valid_flags_trade <- new_valid_flags[flagObservationStatus %in% c('A', 'X', 'E', 'I', 'M'),]
  new_valid_flags_trade_change <- new_valid_flags_trade[,Protected:=FALSE]
  new_valid_flags_trade_change <- new_valid_flags_trade_change[(flagObservationStatus == 'A' &  flagMethod == 'p') |
                                                                 (flagObservationStatus == 'A' &  flagMethod == 'q') |
                                                                 (flagObservationStatus == 'E' &  flagMethod == 'f') |
                                                                 (flagObservationStatus == 'E' &  flagMethod == 'p') |
                                                                 (flagObservationStatus == 'X' &  flagMethod == 'p') , Protected:=TRUE]
  ...
  protected_flags <- new_valid_flags_trade_change[Protected==TRUE,paste(flagObservationStatus, flagMethod)]

  protected_data <-
    existing_data[paste(flagObservationStatus, flagMethod) %in% protected_flags]
  protected_data[, Protected:='TRUE']
  protected_data[, Valid:='TRUE']
  ...
  unprotected_flags <- new_valid_flags_trade_change[Protected==FALSE,paste(flagObservationStatus, flagMethod)]

  data_diff <-
    data_diff[paste(flagObservationStatus, flagMethod) %in% protected_flags]
...
agric_data_tot <-
  agric_data[,
    .(
      ...
      Value = sum(Value),
      flagObservationStatus = "X", # For now? ## It should be 'E' following the SDMX req. Discuss it with the team!
      #  aggregateObservationFlag(
      #    flagObservationStatus,
      #    flagTable = flagWeightTable_status
      #  ),
