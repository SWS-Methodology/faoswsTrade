...
`Trade Flows (CPC)` SWS dataset; the latter takes as input the data
...
calculates the aggregated unit values, and saves data to the `Total
...
the user by email). More information of how the SWS works is given in the
...
The trade module was developed by (in alphabetical order): Marco Garieri,
Alexander Matrunich, Christian Mongeau, Aydan Selek and Bo Werth. Plugins for data
validation have been developed by (in alphabetical order): Carlo Del Bello,
Alberto Munisso, Aydan Selek, Sumeda Siriwardena.  The
interactive tool for data validation has been developed by Christian
Mongeau.  Useful feedback has been given by (in alhpabetical order):
Katherine Baldwin, Claudia De Vita, Carola Fabi, Dominique Habimana, Salar
Tayyib.

Any remaining error/inaccuracy in this document (as of February 2023) can be
attributed exclusively to the authors. A correction and pull request can
be made in the module git repo at:

<https://github.com/SWS-Methodology/faoswsTrade/>


# Raw data processing

The sources of bilateral raw data are the United Nations Statistics Division
(UNSD) for all the countries except the European Union members, and Eurostat for European countries. In
...
Data harvesting of UNSD data is done by using an integrated tool of SWS. The tool has been prepared initially by the Methodological team, and then extended and integrated to SWS by CSI division. On the other hand, the harvesting of Eurostat data is done by using their bulk download facility. The raw data obtained from both data sources can be found in their respective datatable by year under the trade-input-data domain of SWS.

Instructions and links below can be outdated as of this writing.

### UNSD

The SWS harvester of UNSD data uses the UNSD's API (Application
programming interface).  There are two methods: "simple" and bulk
download. The first method is more suitable when there are few
cases to be downloaded. It consists in querying an UNSD webservice
with specific parameters.[^unsdweb] The result will be an SDMX
(Statistical Data and Metadata eXchange)[^sdmx] file that will
need to be converted[^rsdmx] into a tabular format so that it can
then be uploaded to SWS. A list of access points and parameters
can be found at:
<https://unstats.un.org/unsd/tradekb/Print50044.aspx>. For
instance, to download all tariff line data for Colombia's (M49
170) imports and exports of chapter 10 (Cereals) on 2017, the
address is:

<https://comtrade.un.org/ws/getsdmxtarifflinev1.aspx?px=*&p=*&comp=false&r=170&y=2017&cc=01*>

When bigger data needs to be downloaded (e.g., in a new cycle all
available data for all countries will be downloaded), the
preferred method is bulk download. It consists in querying the
API, with its `bulk` method, as described here:

<https://comtrade.un.org/data/doc/api/bulk/#DataRequestsDwn>

As per the link above, "responses are in the form of a ZIP
archive. Each zip archive contains exactly one CSV file. The CSV
file adheres to the format specified in the UN Comtrade API CSV
response format documentation. If new fields have recently been
added to the response format there may be a delay before they
become available in the bulk files."



[^unsdweb]: See <https://comtrade.un.org/data/dev/portal/>.

[^sdmx]: For details on SDMX, see <https://sdmx.org/?page_id=3425>.

[^rsdmx]: An R package that can be used to handle SDMX files is
  rsdmx: <https://CRAN.R-project.org/package=rsdmx>.


### Eurostat

Eurostat data can be obtained through their bulk download
facility:

<https://ec.europa.eu/eurostat/data/bulkdownload>

Specifically, trade data is avialable at **comext** > **COMEXT_DATA** > **PRODUCTS**:

<https://ec.europa.eu/eurostat/estat-navtree-portlet-prod/BulkDownloadListing?sort=1&dir=comext%2FCOMEXT_DATA%2FPRODUCTS>

Relevant files are files named fullYEAR52.7z, where YEAR is a
given year (e.g., full201852.7z), and 52 stands for the last week
of the year. The files are 7-zip compressed files, wich are
compressed files.[^7zip]

[^7zip] These files can be opened with the free 7-Zip software: <https://www.7-zip.org/>.

Note that Eurostat's bulk download facility changed on 2nd
February 2018,[^eurostatbulk] and new data need to be slightly pre-processed
before uploading to SWS. The operations that need to be done are:

* Aggregate by `period`, `declarant`, `partner`, `flow`, `product_nc`, and:
  * `value_1k_euro` = sum(as.numeric(`value_in_euros`) / 1000)
  * `qty_ton` = sum(as.numeric(`quantity_in_kg`) / 1000)
  * `sup_quantity` = sum(as.numeric(`sup_quantity`))
* `supp_unit` = NULL
* `stat_regime` = "4"

[^eurostatbulk]: <https://ec.europa.eu/eurostat/estat-navtree-portlet-prod/BulkDownloadListing?sort=1&file=comext%2F%23%23%23IMPORTANT_NOTE.pdf>
...
```{r, echo = FALSE}
knitr::include_graphics("assets/chapters.png")
```
...
[^shareddrive]: The "R shared drive" is a folder in the SWS server where R
  plugins can write and read data to/from, and can be accessed only through the Rstudio workbench development environment of FAO, with an assigned user account provided by CSI.
...
country as it will be saved in SWS, and a "code" column containing the
original code assigned by the country either by UNSD (M49-comtrade) or
Eurostat (geonom).

### "Reporters by years"

This table reports whether the country is available in the raw data file
...
(and eventually re-imports and re-exports) and indicates it with an "X".
...
and year. This information is encoded as $noqty_F$ and $noqty-prop-_F$, where
...
its output is saved to the "Trade Flows (CPC)" dataset, were
...
in SWS-specific element codes, in the "Trade Flows (CPC)" SWS
...
transactions involving the item with code `22071090`.
...
[^weakconversion]: Probably a weak point of this procedure is that not in
  ...
  to have specific tables derived from external sources. This is currently
  under investigation.
...
corresponding commodities, otherwise it will keep them unmapped. For
...
by the unit value reflecting the annual global price change:

$$quantity_{t,x} = \frac{value_{t,x}}{uv_{t-1,x} * \frac{uv_{t,global\text{ } average}}{uv_{t-1,global\text{ }average}}}$$

where $x$ represents the country and $t$ represents the year in subject. To have a validated and better global price change, the countries with missing quantities are validated last among the reporter countries.

The global prices changes of import and export, together with more detailed information on regional changes, can be found in ESS trade global UVs datatable in SWS. More details on this datatable will be given below.
...
instance, if a country reports quantity data for 19 out of 20 partners, but
...
but arrives at destination in January); transaction may have been reported
...
considered to have incomplete or low quality data (for any reason). This is
...
- $value / value_{mean} < 0.4$
- $quantity / quantity_{mean} < 0.4$
- $quantity_{mean} > 1,000$ tonnes

where $value_{mean}$ and $quantity_{mean}$ are the 3-year average of the
...
its three partners, as the partners declared; the lower part contains the
...
However, these represent only around 35% of what is usually declared by the
country (making the comparison with its 3-year average). Thus, according to
...
When the plugin removes mirrored flows because they did not meet the 40%
...
order to assign them, are reported in the table below.


```{r, echo = FALSE}
knitr::include_graphics("assets/tradeflags.png")
```
...
value prevails over the other flags. For instance, if one shipment of
...
transaction (specifically, 7.8%), while the flag for Item B is "I" because
...
The plugins usually require that the user opens a session in the
...
"Geographic Area M49" dimension (remaining dimensions can be set
arbitrarily), then their "Run" button needs to be selected.
...
Running the plugins will allow the analyst understand which flows at the
total level require attention. As the analyst collects a list of
"suspicious" data at the total level, he/she can proceed the investigation
and eventually correction at the bilateral level with the interactive
validation tool (Shiny tool).

## List of validation plugins

### `trade_outlier_detection` plugin

This plugin calculates the 5-year average value of validated years for each
...
This plugin calculate the ratio of the current year quantity and divides it
...
* blue cells: $quantity / movav > 2$, i.e., Values minimum twice more than the 5-year average
...
This plugin sorts in decreasing order commodities by quantity for
non-reporting countries, showing which items have been deleted by
the "TP criterion" (see the "Discarding incompatible mirror data
("TP criterion")" Section), or are relatively low when compared to
...
When the plugin completes, it sends an email with subject
"main_commodities" with an Excel that indicates:

* yellow cells: values less than 50% of their 5-year average
* blue cells: values minimum twice more than the 5-year average
* red cells: missing or deleted values considered to be incompatible with their time series ("bad TP")
* boldface: official data

```{r, echo = FALSE}
knitr::include_graphics("assets/validation_top_excel.png")
```

### `monetary_values_ranked_commodities` plugin

This plugin is used for the validation of total
agricultural/merchandise trade data. It calculates and ranks
5-year averages of the monetary values of all items and facilitate
the comparison of "Total Merchandise Trade" (CPC F1881) and
"Agricult.Products,Total" (CPC F1882) by adding the "SHARE of
Agriculture to Merchandise" ($F1881 / F1882 \times 100$).

```{r, echo = FALSE}
knitr::include_graphics("assets/validation_monetary_popup.png")
```

When the plugin completes successfully, it sends an email with
subject "Monetary values ranked commodities" with an Excel that
indicates:

* yellow cells: values less than 50% of their 5-year average
* red cells: *SHARE of Agriculture to Merchandise* is greater than 50%
* pink cells: missing value
* boldface: official data

```{r, echo = FALSE}
knitr::include_graphics("assets/validation_monetary_excel.png")
```

### `trade_commodity_tables` plugin

This plugin is designed to validate imports and
exports nat the global level. It runs only for selected items,
available in the `commodity_list_for_total_trade_tables` datatable.

```{r, echo = FALSE}
knitr::include_graphics("assets/validation_commodity_popup.png")
```

The plugin sends two emails as output. The first one is sent to
the analyst and has subject "World" and contains an Excel file
with global trade for all the items: red cells indicate that there
is no balance between imports and exports (+/- 5%).

```{r, echo = FALSE}
knitr::include_graphics("assets/validation_commodity_excel_1.png")
```

The second email is sent to the officer and contains the single
commodity tables for each item. These Excel files have three sheets,
which are "World_summary", "Country_details", and "Regions", with
details of global, country and region, respectively.

```{r, echo = FALSE}
knitr::include_graphics("assets/validation_commodity_excel_2.png")
```
...
<https://rstudioc.fao.org/TRADEvalidation/>
...
1. create a session in the "Total Trade (CPC)" dataset
    ...
    1. rename the session to something that you remember (here it will be "trade_CO_TOT")  
       ...
       ```
1. create a session in the "Trade Flows (CPC)" dataset
  ...
  1. copy the token  
     ...
     ```
1. open <https://rstudioc.fao.org/TRADEvalidation/>
...
2) the partner should be automatically populated if you select it in the bilaterals that appear in the "Total" tab.
...
What this action will do is that all corrections will be stored in the "corrections table",
...
If you find, e.g., that ALL partners' quantities are to be divided by 1,000,
this may be better handled at the source of the data.
It could be useful to liaise with UNSD so that in a coordinated manner these flows get fixed.
...
is done, the developer needs to update the version in the server. To this so, the developer needs to have FAO R Studio Connect account and publish the updated version using local R studio connected to this account. <https://rstudioc.fao.org/connect/>
...
The steps required to complete one full round of the trade process so to
save data in the appropriate trade datasets are the following (validation
will be covered in the "Data validation" section):

1. Open a query in the "Trade Flows (CPC)" dataset of the "Trade"
...
2. Select the "[FAODOMAIN/Terrinoni] Complete Trade Flow CPC - from QA (core)" plugin,
...
3. Once the "Complete Trade Flow (CPC)" plugin has completed successfully,
   open a query in the "Total Trade (CPC)" dataset of the "Trade" domain (as
...
4. Select the "[FAODOMAIN/Terrinoni] Total Trade CPC - from QA (core)" plugin,
   set the year the plugin needs to process data, and click "Run plugin":

   ```{r, echo = FALSE}
   knitr::include_graphics("assets/runs_total_run.png")
   ```
...
6. Validation by country analysts starts, as described in the next subsection.
...
performance, the amount of data, and other technical factors).

## Validation flow

Once the data has been saved for all years, as described in the previous
subsection, analysts will start the data validation process. It differs on
whether the validation needs to be done for reporting countries, or
non-reporting countries. Also, validation of commodity tables need to be
taken into account. Validation occurs in the following order:

1. reporting countries
1. non-reporting countries
1. total merchandise trade
1. commodity tables

At the beginning of the data validation exercise, a "monitoring table" will
be compiled by the trade officer. It contains the distribution of countries
for the first 3 validation phases (reporting countries, non-reporting countries, and 
total merchandise trade), and the distribution of commodities (for commodity
tables). Given that the process is sequential, not necessarily the
distribution for all steps will be available at the very beginning of the
cycle.

In some of the steps involved in validation, the analyst will need to check
for external sources of data. Some examples of where to find additional data
are:

* TRADEMAP: <https://www.trademap.org/>
* FAO-AMIS: <https://app.amis-outlook.org/#/market-database/supply-and-demand-overview>
* USDA: <https://apps.fas.usda.gov/psdonline/app/index.html#/app/advQuery>
* National Statistics Offices
* Publications available on the web or in FAO library (e.g., "Oil World")

Documents with more details on the process will be circulated by the trade
officer at the beginning of the cycle.

### Reporting countries

This step will show the analyst outliers at the total level, by running the
plugin:

`trade_outlier_detection`

when it completes, and email will be sent with a list of outliers. The
analysts is required to have a look at the historical time series of all
items contained in the email to check whether the identified outlier is a
true positive (i.e., an actual outlier) and, thus, it deserves attention.
For doing this, there are two options:

* open a session in the "Trade Flows (CPC)" dataset with the
  required reporter/item/flow/years combination, making sure to include all
  partners (as for years, it would be useful to have data for at least five
  years previous of the identified outlier).
* open that Shiny tool, as explained in the "Interactive tool for data
  validation (Shiny tool)" Section.

Given that in any case the actual validation of bilateral data happens
through the Shiny tool, the second option is preferred.

The analyst is required to use the tools available in the Shiny tool to make
corrections to bilateral data, and keep comments on the actions made. These
can be saved in the Excel file sent by the
`trade_outlier_detection` plugin. Taking notes will help
post-validation checks by officers.

If, for some valid reason, bilateral flows cannot be reconciled with the sum
of bilateral flows even after these are validated, the analyst have the
option to override the aggregated total obtained from bilateral data, and
input some external figure, considered to be more in line with the country
historical trend. In this case, the analyst need to write the appropriate
number in the corresponding cell on the SWS session on the total dataset,
and change flags in an appropriate way (e.g., if the data comes from a
national statistical office, the flag combination needs to be changed to
X,p), and click on "Apply":

```{r, echo = FALSE}
knitr::include_graphics("assets/sws_change_data_flags.png")
```

Once this operation is done, a red triangle will appear, indicating it is
"session data". The analyst will need to click on th "Save to database"
button (visible in the previous screenshot in the upper-right part, third
row).

### Non-reporting countries

As the validation of reporting countries is completed, the analysts will be
required to validate non-reporting (thus, *mirrored*) countries. The
following plugin:

`top_commodities_selection_routine`

will read data for the required non-reporting and will send an email with a
list of deleted flows (because they were considered not to match historical
time series for that partner, see the "Discarding incompatible mirror data ("TP
criterion") Section"), or flows that are relatively small, compared to
previous data.

Analysts will need to find external data sources for **all deleted flows**
(highlighted with red cells in the email sent by the plugin) or, though in
rare occasions, estimate themselves what the flow should be.

It may happen that external sources confirm the removed figure (e.g., a
drought affected seriously the country and the exports are much lower
compared to its standards). If this is the case, the analysts will need to
"restore" a figure which comes with Excel.

After data has been restored, a red triangle will appear, indicating it is
"session data". The analyst will need to click on th "Save to database"
button (visible in the previous screenshot in the upper-right part, third
row).

Please note that non-reporting countries cannot be validated in the Shiny
tool. This is because the tool is meant to be used in the validation of
reporters.

Some outliers may have been missed by analysts that worked on reporting
countries or, more likely, what is not an outlier (or important one) for a
reporter, may be so for a non-reporting country. Thus, a check for the
presence of outliers will be required by using the following plugin:

`trade_outlier_detection`

Once outliers are identified, the analyst will need to check if the
trade flows dataset that creates the outlier at the total level is an
outlier also for the country that originally reported that flow, or if it is
an outlier only for the non-reporting country that is being analysed. In the
former case, the analyst need to contact the colleague that had that country
under his/her responsibility and ask for this to be re-validated. In the
latter case, the analyst will need to contact the trade officer and agree
on a solution.

### Total merchandise trade

`monetary_values_ranked_commodities`

### Commodity tables

`trade_commodity_tables`

# Future work

## Streamline raw data updates

As of January 2023 the process of checking whether a country that in
...
neighbor or similar countries (e.g., the median unit value of Asian
...
scheduled runs, and for the reasons explained in the issue, it seems
...
(known more frequently by CSI and other divisions as
...
statistical (by ESS and other technical units), and information technology
(by CSI) points of view. These are the URL from accessing the two servers,
from the web and R-API:

* QA:
  * web: https://sws.qa.fao.org/
  * R-API: https://sws.qa.aws.fao.org:8181
* LIVE (production):
  * web: https://sws.fao.org/
  * R-API: https://sws.aws.fao.org:8181
...
domain: "Trade Flows (CPC)" and "Total Trade (CPC)". As the names
...
shown, where it can be seen that 2014 and 2015 are available.
...
This datatable contains the items used by the
`trade_commodity_tables` plugin for doing the "Commodity tables".

Note that this mapping also includes:

* the "European Union" aggregate, which includes 28 countries EU countries
* the "Europe" aggregate, which excludes 28 countries that are member of European Union
* the "Asia" aggregate, which excludes one country (Cyprus - 196) that is also a member of "Europe"
...
This datatable contains a list of country-continent correspondences.
...
These datatables have already been presented in the Section "HS-FCL-CPC code
mapping". They contain the correspondences of country HS codes to the FCL
and CPC, by *YEAR*.
...
The correspondences between FAO area codes and geonomenclature (Eurostat
...
"correction_X" column, where *X* is the variable that needs to be corrected
...
There are other datatables with the name corresponding to the previous one, but
...
1. Create a new query to the appropriate dataset (i.e., where the plugin
   runs; in the example it is the "Trade Flows (CPC)" dataset of
...
3. Choose one of the scheduling options available, namely:
   ...
   * *Every*: the plugin will run each day at the specific time (in the
     ...
     time).
   * *Custom schedule (advanced)*: this is a more advanced scheduling
...
and read data to/from, and can be accessed only by the persons who has FAO R Studio Workbench account, through this link: <https://rstudios.fao.org/>.

The location in the SWS server, where R can read from and write to, is `/work/SWS_R_Share`.
...
The UNSD and Eurostat raw tables (for years 2014-2021) are stored in the
...
need to do these infrequent synchronizations.
