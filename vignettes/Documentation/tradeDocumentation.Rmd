...
Alexander Matrunich, Christian Mongeau, and Bo Werth.  Plugins for data
validation have been developed by (in alphabetical order): Carlo Del Bello,
Alberto Munisso, Aydan Selek, Sumeda Siriwardena, Cristina Valdivia.  The
...
Any remaining error/inaccuracy in this document (as of June 2020) can be
attributed exclusively to the author. A correction and pull request can
...
Data harvesting is done by the Methodological team. The
explanation of the process below is temporary as the team finish
documentation of their harvesters. Instructions below can be
outdated as of this writing (June 2020).

### UNSD

UNSD data can be obtained through UNSD's API (Application
...
**NOTE: please double check this, by downloading a previous year,
applying the operations and compare them with data already saved
in SWS Eurostat datatables.**
...
`Tradeoutlier_most_recent_version`
...
* open a session in the "Bilateral Trade Flow (CPC)" dataset with the
...
`Tradeoutlier_most_recent_version` plugin. Taking notes will help
...
input some exteranl figure, considered to be more in line with the country
...
"restore" a figure, i.e., selecting the deleted figure and selecting
"History" in the lower part of the SWS UI, then the appropriate history
entry, then click the restore symbol, as shown in the following Figure:

```{r, echo = FALSE}
knitr::include_graphics("assets/sws_restore.png")
```
...
bilateral flow that creates the outlier at the total level is an
...
latter case, the analyst will need to contact the trade officer and agreen
on a solution.
...
Note that that this mapping also includes:
